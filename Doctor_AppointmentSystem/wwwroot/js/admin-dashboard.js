...
const dash = window.adminDashboardData || {};

// ✅ Revenue (rolling 12 months) - dynamic only
const labels12 = Array.isArray(dash?.revenueLabels) ? dash.revenueLabels : [];
const revenue12 = Array.isArray(dash?.revenue) ? dash.revenue : [];

// ✅ Appointments (keep UI as last 6 months)
let apptLabelsForChart = Array.isArray(dash?.apptLabels) ? dash.apptLabels : [];
let apptDataForChart = Array.isArray(dash?.appointments) ? dash.appointments : [];

if (apptLabelsForChart.length > 6) apptLabelsForChart = apptLabelsForChart.slice(-6);
if (apptDataForChart.length > 6) apptDataForChart = apptDataForChart.slice(-6);

// REVENUE CHART – BAR
const revCanvas = document.getElementById('revenueChart');
if (revCanvas && labels12.length && revenue12.length) {
